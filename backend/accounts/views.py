...
def generate_ai_bio(request):
    ...
    [작성 규칙]
    1. 반드시 한국어로 작성하고, 공백 포함 20자로 작성
    2. 취향을 단순 나열하지 말고, 이 유저의 영화적 정체성을 닉네임처럼 표현할 수 있는 하나의 문장으로 정의
       (예: 'B급 감성을 사랑하는 냉철한 분석가', '로맨스 영화에 진심인 눈물 많은 관객' 등)
    3. 활동 데이터가 부족하다면, 선호 장르를 고려해서 이제 막 취향을 찾아가는 중이라는 컨셉으로 작성
    4. 리뷰의 말투(냉소적, 열정적, 간결함 등)가 있다면 소개 문구에도 그 분위기를 반영
    5. 예시는 참고용일 뿐 출력에 포함 금지
    6. 따옴표, 이모지나 불필요한 설명 없이 문장만 출력
    """
